...
                {{-- ACTIVE CHAT --}}
                <div id="seller-active-chat" class="hidden flex-col h-full opacity-0 transition-opacity duration-300 relative">
...
                    </div>
                    ...
                    <button id="new-message-toast" onclick="scrollToBottom(false)" class="hidden absolute bottom-[80px] left-1/2 -translate-x-1/2 bg-blue-600/90 backdrop-blur-sm text-white px-4 py-2 rounded-full shadow-[0_10px_20px_rgba(37,99,235,0.3)] text-[10px] font-bold tracking-wider uppercase z-50 animate-bounce hover:bg-blue-700 transition-colors border border-blue-500/50 flex items-center gap-2 outline-none">
                        Ada Pesan Baru <i class="fas fa-arrow-down text-blue-200"></i>
                    </button>
...
        startSmartPolling();
        ...
        // Event listener saat user scroll, untuk menyembunyikan toast kalau sudah nyampe bawah
        document.getElementById('seller-chat-messages').addEventListener('scroll', function() {
...
        let html = '';
        if(data.length === 0) {
            html = `<div class="p-6 text-center text-zinc-400 text-xs font-medium">Belum ada obrolan. Cari produk dan klik chat!</div>`;
        } else {
            data.forEach(toko => {
...
            });
        ...
        // PENTING: Hanya render ulang DOM jika HTML benar-benar berubah untuk mencegah kedip
        const newHtml = html.trim();
        if (container.getAttribute('data-raw-html') !== newHtml) {
            container.innerHTML = newHtml;
            container.setAttribute('data-raw-html', newHtml);
        }
    }
...
        currentStoreId = storeId;
        currentMessageCount = -1; // Reset jumlah pesan agar initial load berjalan sempurna
...
            const msgContainer = document.getElementById('seller-chat-messages');
            ...
            // 1. Kondisi Pertama Load Chat
            if (isInitialLoad || currentMessageCount === -1) {
...
                currentMessageCount = data.length;
                ...
                    data.forEach(msg => appendSellerMessage(msg.content, msg.sender, msg.time, msg.type, msg.fileName, msg.is_read, false));
                }

                setTimeout(() => { scrollToBottom(true); }, 50);

            // 2. Kondisi Ada Pesan Masuk (Polling)
            } else if (data.length > currentMessageCount) {
                const newMessages = data.slice(currentMessageCount);
                if (currentMessageCount === 0) msgContainer.innerHTML = ''; // Hapus tampilan kosong

                newMessages.forEach(msg => appendSellerMessage(msg.content, msg.sender, msg.time, msg.type, msg.fileName, msg.is_read, true));
                currentMessageCount = data.length;

                // Cek apakah user lagi nge-scroll ke atas
                const isScrolledUp = (msgContainer.scrollHeight - msgContainer.scrollTop - msgContainer.clientHeight) > 250;
            ...
            // 3. Kondisi Cek Checklist Centang (Read)
            } else if (data.length === currentMessageCount) {
                updateReadTicks(data);
            }
            // Jika data.length < currentMessageCount (biasanya terjadi karena jeda respon saat send pesan lokal), kita diamkan saja agar tidak kedip.
        } catch(e) {
            if(isInitialLoad) {
...
    }
    ...
        const msgContainer = document.getElementById('seller-chat-messages');
        const toast = document.getElementById('new-message-toast');
        if (toast) toast.classList.add('hidden'); // Sembunyikan toast pas turun
