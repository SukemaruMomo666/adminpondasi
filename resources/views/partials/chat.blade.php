...
                } else {
                     // Agar saat load pertama kali TIDAK TER-ANIMASI (tidak slide in/scroll), berikan !isInitialLoad
                     data.forEach(msg => appendSellerMessage(msg.content, msg.sender, msg.time, msg.type, msg.fileName, msg.is_read, !isInitialLoad));
                }

                // Gunakan setTimeout kecil untuk menunggu DOM selesai merender innerHTML sebelum melompat ke bawah
                setTimeout(() => {
                    if(isInitialLoad || (msgContainer.scrollHeight - msgContainer.scrollTop - msgContainer.clientHeight < 150)) {
                        scrollToBottom(isInitialLoad); // saat masuk isInstant = true, saat pesan baru isInstant = false
                    }
                }, 10);
...
    }
    ...
        const msgContainer = document.getElementById('seller-chat-messages');
        if (isInstant) {
            // Gunakan scrollTop langsung agar instan 100% tanpa animasi geser
