...
@push('styles')
<style>
    /* Styling scrollbar & smooth scroll */
    .chat-scrollbar { scroll-behavior: smooth; }
    ...
    .animate-slide-right { animation: slide-in-right 0.3s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
    .animate-slide-left { animation: slide-in-left 0.3s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
</style>
@endpush
        ...
        <div id="chatPanel" class="absolute md:relative top-0 left-0 w-full h-full md:flex flex-1 flex-col bg-slate-100 z-20 md:z-auto transition-transform duration-300 translate-x-full md:translate-x-0 hidden md:block">
            ...
            <div id="chatActiveWindow" class="hidden flex-col h-full bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-slate-50">
                ...
                <div id="messageArea" class="flex-1 p-4 sm:p-6 overflow-y-auto chat-scrollbar flex flex-col gap-4 relative">
                    <!-- Balon pesan di-inject JS ke sini -->
                </div>
                ...
                <div class="bg-white border-t border-slate-200 flex flex-col flex-shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] relative z-20">
                    <div id="media-preview-container" class="hidden items-center justify-between p-3 mx-4 mt-3 bg-slate-50 border border-slate-200 rounded-xl shadow-inner">
                        <div id="media-preview-content" class="flex items-center gap-3 w-full overflow-hidden"></div>
                        <button type="button" onclick="cancelMediaPreview()" class="w-8 h-8 rounded-full bg-red-100 text-red-500 hover:bg-red-200 flex items-center justify-center transition-colors flex-shrink-0 ml-2 shadow-sm"><i class="mdi mdi-close"></i></button>
                    </div>

                    <div class="flex items-center gap-3 px-5 py-2.5 bg-white border-b border-slate-100">
                        <button type="button" onclick="document.getElementById('upload-image').click()" class="text-slate-400 hover:text-blue-600 transition-colors outline-none"><i class="mdi mdi-image-outline text-xl leading-none"></i></button>
                        <button type="button" onclick="document.getElementById('upload-file').click()" class="text-slate-400 hover:text-blue-600 transition-colors outline-none"><i class="mdi mdi-paperclip text-xl leading-none"></i></button>
                        <button type="button" id="record-vn-btn" onclick="toggleVoiceNote()" class="text-slate-400 hover:text-red-500 transition-colors outline-none relative ml-1">
...
<div id="image-lightbox" class="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/90 backdrop-blur-sm p-4 transition-opacity duration-300 opacity-0" onclick="closeLightbox()">
    <button class="absolute top-6 right-6 text-white text-4xl hover:text-red-500 transition-colors outline-none z-50"><i class="mdi mdi-close"></i></button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.closeLightbox = function() {
        ...
        setTimeout(() => { lightbox.classList.remove('flex'); lightbox.classList.add('hidden'); }, 300);
    }
    ...
    // PUSHER WHISPER: TYPING INDICATOR REALTIME
    msgInput.addEventListener('input', function() {
        if(activeChatId && echoChannel) {
            window.Echo.private(echoChannel).whisper('typing', { is_typing: true });
        }
    });
    ...
    contactListDiv.addEventListener('click', function(e) {
        ...
        window.Echo.private(echoChannel)
            .listen('PesanBaruTerkirim', (e) => {
                appendMessageUI(e.message.content, false, e.message.time, e.message.type, e.message.file_name, 0, true);
                scrollToBottom();
                loadChatList();
            })
    ...
    function loadMessages(chatId, isInitialLoad = false) {
        if(!chatId) return;
        fetch("{{ route('seller.service.chat.messages', ':id') }}".replace(':id', chatId))
            .then(res => res.json())
            .then(data => {
                let items = data.data || data;
                ...
                items.forEach(msg => {
                    ...
                    // Disable animasi saat loading awal agar tidak glitch
                    appendMessageUI(msg.content || msg.text, isOut, msg.time, msg.type, msg.fileName, msg.is_read, false);
                });
    ...
    function appendMessageUI(content, isOut, time, type = 'text', fileName = '', isRead = 0, animate = true) {
        ...
        if(isOut) {
            tickHtml = isRead == 1
                ? `<div class="chat-status flex items-center gap-1 bg-white/25 text-white px-1.5 py-[2px] rounded uppercase ml-1"><span class="text-[8px] tracking-wider font-black">SEEN</span><i class="chat-tick mdi mdi-check-all text-[11px] leading-none"></i></div>`
                : `<div class="chat-status flex items-center gap-1 text-blue-200 px-1.5 py-0.5 ml-1"><i class="chat-tick mdi mdi-check text-[11px] leading-none"></i></div>`;
        }

        let innerHTML = '';
        if(!type || type === 'text') innerHTML = content;
        else if (type === 'image') innerHTML = `<div class="p-1"><img src="${content}" onclick="openLightbox('${content}')" class="max-w-[200px] md:max-w-[250px] rounded-xl object-cover hover:opacity-90 transition-opacity cursor-pointer shadow-sm"></div>`;
        else if (type === 'file') innerHTML = `<a href="${content}" download="${fileName}" class="flex items-center gap-3 hover:bg-black/10 transition-colors rounded-xl no-underline p-1"><div class="w-10 h-10 rounded-lg bg-red-100 flex items-center justify-center text-red-600 shrink-0"><i class="mdi mdi-file-pdf-box text-2xl"></i></div><div class="flex flex-col min-w-0 pr-2"><span class="text-xs font-bold truncate max-w-[150px]">${fileName || 'Dokumen'}</span><span class="text-[9px] uppercase tracking-widest mt-0.5 opacity-80"><i class="mdi mdi-download"></i> Unduh File</span></div></a>`;
        else if (type === 'audio') innerHTML = `<div class="w-[200px] md:w-[240px] pt-1"><audio controls src="${content}" class="w-full custom-audio"></audio></div>`;

        let html = `
            <div class="message-bubble flex flex-col max-w-[85%] md:max-w-[75%] ${wrapAlign} ${transformOrigin} ${animClass} opacity-100">
    ...
    function sendMediaMessage(content, type = 'text', fileName = '', caption = '') {
        ...
        fetch("{{ route('seller.service.chat.send') }}", {
            ...
            body: JSON.stringify({ chat_id: activeChatId, message: content, type: type, file_name: fileName })
        }).then(res => res.json()).then(data => { loadChatList(); }).finally(() => {
            btn.disabled = false; btn.innerHTML = '<i class="mdi mdi-send text-lg leading-none"></i>'; msgInput.focus();
        });
    }
    ...
    window.handleFileUpload = function(inputElement, type) {
        ...
        if(file.size > 2000000) return alert('Maksimal ukuran file adalah 2MB.');
        const reader = new FileReader();
        reader.onload = e => showMediaPreview(type, e.target.result, file.name);
        reader.readAsDataURL(file); inputElement.value = '';
    }

    let vnRecorder; let vnChunks = []; let isRecordingVN = false;
    window.toggleVoiceNote = async function() {
        if(!activeChatId) return alert('Pilih chat terlebih dahulu!');
        const btn = document.getElementById('record-vn-btn'), ping = document.getElementById('vn-ping'), indicator = document.getElementById('recording-indicator');
        if(isRecordingVN) {
            vnRecorder.stop(); isRecordingVN = false;
            btn.classList.remove('text-red-500'); btn.classList.add('text-slate-400');
            ping.classList.add('hidden'); indicator.classList.add('hidden'); indicator.classList.remove('flex');
        } else {
            try {
                const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
                vnRecorder = new MediaRecorder(stream); vnChunks = [];
                vnRecorder.ondataavailable = e => { if (e.data.size > 0) vnChunks.push(e.data); };
                vnRecorder.onstop = () => {
                    const audioBlob = new Blob(vnChunks, { type: 'audio/webm' });
                    const reader = new FileReader();
                    reader.onload = e => showMediaPreview('audio', e.target.result, 'Voice Note');
                    reader.readAsDataURL(audioBlob); stream.getTracks().forEach(t => t.stop());
                };
                vnRecorder.start(); isRecordingVN = true;
                btn.classList.add('text-red-500'); btn.classList.remove('text-slate-400');
                ping.classList.remove('hidden'); indicator.classList.remove('hidden'); indicator.classList.add('flex');
    ...
    function showMediaPreview(type, content, fileName) {
        ...
        const container = document.getElementById('media-preview-container'), contentDiv = document.getElementById('media-preview-content');
        if(type === 'image') contentDiv.innerHTML = `<img src="${content}" class="h-12 w-12 object-cover rounded-lg border border-slate-200 shadow-sm"> <div class="flex flex-col"><span class="text-xs font-black text-slate-800 truncate">Gambar Siap Kirim</span></div>`;
        else if (type === 'file') contentDiv.innerHTML = `<div class="h-12 w-12 bg-red-100 text-red-500 rounded-lg flex items-center justify-center text-xl shadow-sm"><i class="mdi mdi-file-pdf-box"></i></div> <div class="flex flex-col"><span class="text-xs font-black text-slate-800 truncate max-w-[200px]">${fileName}</span></div>`;
        else if (type === 'audio') contentDiv.innerHTML = `<div class="h-12 w-12 bg-blue-100 text-blue-500 rounded-lg flex items-center justify-center text-xl shadow-sm"><i class="mdi mdi-microphone"></i></div> <div class="flex flex-col"><span class="text-xs font-black text-slate-800">Voice Note</span></div>`;
        container.classList.remove('hidden'); container.classList.add('flex');
    }

    window.cancelMediaPreview = function() {
        pendingMedia = null; document.getElementById('media-preview-container').classList.add('hidden');
    }

    document.getElementById('btnBackMobile').addEventListener('click', function() {
        pnlChat.classList.add('hidden', 'translate-x-full'); pnlChat.classList.remove('flex', 'translate-x-0');
        if(echoChannel) { window.Echo.leave(echoChannel); echoChannel = null; }
        activeChatId = null;
